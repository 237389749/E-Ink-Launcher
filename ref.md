...
| # | 做法 | 效果 | 代价 |
|---|---|---|---|
| 1 | ~~**降低操作频率**（翻页间隔 >1.5s）~~ | ❌ **已否证** —— 见 ⑫：间隔与积压**无系统性关联**（0.35s 反而最好）| — |
...
> ⚠️ **本表第 1 项已于同日被否证**（详见 ⑫）—— 读本节时请以 ⑫⑬⑭ 为准：**软件层没有稳定解法**。
...
**⑫ ★★★ 更正（同日后续验证）：**「降低操作频率」也是**无效**的 —— ⑦ 的结论被否证**

对 ⑦ 的「唯一稳定有效」做**间隔扫描**（翻页间隔分 6 档，各 6 次，顺序打乱以排除漂移）：
...
⇒ **积压深度与操作间隔【无系统性差异】**；**最密的 0.35s 两次都是最好的**（`maxLut=1`）。
⇒ ⇒ **⑦ 的「降低操作频率」（翻页间隔 >1.5s）【不成立】** —— 那是从「0.35s 快翻积压 / idle 不积压」的**跨窗口**对比错误外推的。

**★ 真正的原因（本条的关键证据）：积压由 `powerup` 失败驱动，与操作节奏无关**

| 实验窗口 | uptime | 对应 kt | 该窗口 `Reg Enable` | 实测积压 |
|---|---|---|---|---|
| 早先「10 次 0.35s 快翻」 | 37299~37342 | 9610~9653 | **落在 kt 9500~10000 最密集桶** | `luts` 峰值 **7** |
| 本轮间隔扫描 | 40179~40337 | 12490~12647 | **= 0**（1947 行 dmesg 里 1889 行是 `dump_lcdc_state`）| `maxLut` **1~3** |

⇒ **同一操作节奏，在「有 powerup 失败」的窗口积压、在「无 powerup 失败」的窗口不积压。**
⇒ 本轮 6 档全部干净，是因为实验期间 **EPDC 零 powerup 尝试**（设备已连续约 8 小时无 `Reg Enable`）。

**⑬ 两个指标的可信度修正（避免继续误用）**

| 指标 | 问题 | 结论 |
|---|---|---|
| 「最长停滞」（frame 两次变化的最大间隔）| **被操作间隔主导** —— 5.0s 档测出的「8.05s 停滞」其实 ≈ 操作间隔本身 | ⚠️ **该指标在带操作间隔的实验中无效**，只能在 idle / 固定节奏下比较 |
| `maxLut`（`epdc_active_luts` 第 4 槽）| 与 powerup 活动强相关 | ✅ **可用**（本条的主指标）|

**⑭ 对「还有办法解决吗」的最终答案**

| 方向 | 实测 |
|---|---|
| 「减缓流程内的时间」（debouncer / merge / setUpdListSize）| ❌ **无效**（③⑤）|
| 「延长触发间隔」（0.35s → 5.0s）| ❌ **也无效**（⑫）|
| **真正的决定因素** | **硬件供电状态（时间随机）** —— 有 `powerup` 失败才积压 |

⇒ **软件层没有稳定解法。** 积压的可控性受限于 TPS6518x 的供电成功率，而它随机且不可从用户态干预。
⇒ 唯一根治仍是**修硬件**（⑦ 第 4 项）。
